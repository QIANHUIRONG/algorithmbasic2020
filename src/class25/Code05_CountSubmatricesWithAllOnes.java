...
/*
题意：统计全1子矩形
你一个 m x n 的二进制矩阵 mat ，请你返回有多少个 子矩形 的元素全部都是 1 。
...
/*
思维导图：
1、大流程：第0行做底的子矩形，全是1的有多少个；第2行做底的子矩形，全是1的有多少个；累加。这样求解不会多也不会少
2、以某一行做底时：
	1.求必须以0位置的7做高的情况下，全是1的子矩形有多少个？求必须以1位置的8做高的情况下，全是1的子矩形有多少个？...所有的累加
	2.x位置的y做高，利用单调栈，左右两边到不了的位置是l,r,那么我要统计x位置的y能扩出来的区域所有的达标子矩形, 那么全是1的子矩形数量是。 （首项+尾项） * 项数 / 2
	b	y	d
	a	x	c
	x位置的y，左边扩不到的位置是a位置的b，右边扩不到的位置是c位置的d，那么x扩出来的区域有多少达标子矩形？
	高都是y，长是a+1, a+1-a+2, a+1-...-a+c-1;
			长是a+2，a+2-a+3, a+2-...-a+c-1,
			所以一共是1+2+3+4+。。。+n
	如果扩出来的长度是l的话，子矩形的数量是：（1+l) * l / 2

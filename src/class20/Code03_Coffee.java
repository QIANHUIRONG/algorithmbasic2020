 ...
 */
/*
1.分析可变参数，弄出表的大小，如果这一步你弄不出表的大小，一定是有一个可变参数的范围你不好估计，你就按照它能够冲到的最大程度来估计，你写的递归
尝试，如果一个参数的可变范围你不知道，就假设最差情况去算它，看能冲多大
2.再填DP表的时候，有一个很重要的点，你一定要关心哪些位置是你不需要填的，比如说范围的尝试，为什么左下半区你不需要填，因为如果是真实的一个从L…R
的范围的话，左下半区一定不会被我的递归调到，所以我不填它们
	喝咖啡洗咖啡杯问题的这道题也一样，如果我已经告诉你了，洗20杯咖啡，最疯的情况下，咖啡杯的机器到100的时间点出来，
当你遇到第5杯，超过100绝对不会被你用到，你越界是因为啥？你正常不管到哪一杯咖啡，没有任何一个时刻，这个洗咖啡的机器居然在100以外，如果你在决策中居然有越界了，啥情况？说明就目前为止你
要做的这个决策，所用的这个状态，一定不会被你递归调到，这个时候，不填它.它本质就和我们在范围上尝试L>R的时候不需要填，一码事
3.如果你实在改不出严格动态依赖的话，就傻缓存
 */

...
定义：
	...
	3.用户可以在任何时候调用合并和查询两个方法：
		boolean isSameSet(Vx,Vy):查询样本x和样本y是否属于个集合
		void union(Vx,Vy):把x和y各自所在集合的所有样本合并成一个集合

时间复杂度：单次均难调用代价O(1)。（记住就行，这个证明是数学上的问题，非常复杂）
均摊：比如有100w的数据，调用union和isSameSet调用非常频繁，可能1000w次，平均每一次o(1)

连通性问题：想到并查集。比如朋友圈、岛问题等

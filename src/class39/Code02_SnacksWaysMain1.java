...
2^30,10^8拿不下。所以用分治，左侧15个零食去算，右侧15个零食去算，收集一张表：左侧这15个零食产生的所有可能的累加和的方法数。
你干出这么一张表，就是2^15 + 2^15肯定在10^8以内
2.func(int[] arr, int index, int end, long sum, long bag, TreeMap<Long, Long> map){}
从index出发，到end结束。之前的选择，已经形成的累加和sum，零食[index....end]自由选择，出来的所有累加和，
返回累加和不超过bag的方法数，每一种累加和对应的方法数，填在map里
3.主流程还是index位置要和不要两种情况，只不过要收集方法数到map种
4.主函数：
...
生成一个类似于这样前缀和的表，但是你的Key只用记住1,2,6就可以了
	...
	和左侧累加和严格为20适配上的所有东西不一样，它可以保证方法不要重算
...
我们就可以根据输入数据的状况，启发自己从什么角度想，或者自己想了一个办法看看，如果超过了108就直接杀死，

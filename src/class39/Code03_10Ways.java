...
一、卡特兰数
    1.k(0)=1,k(1)=1时，如果接下来的项满足：
     ...
     就说集合A和集合B的数量一样多，他俩是等势的。
     	3.一些等势的例子：整数和偶数一样多；直线和平面的点数一样多
     ...
     3.左右括号各N个组合出来的括号配对是否合法
     	...
     	2.直接算有多少合法比较困难，可以去求有多少不合法的：
     		...
     		你在前缀上是右括号比左括号多一个。我把后面所有的括号反转了，反转之后所形成的样子是右括号整体会比左括号多两个。
     		这就是B集合：N+1右括号，N-1个左括号所形成的的所有样子。这个映射关系就是A集合->B集合。
     		而且是唯一的，就是A集合通过这个映射关系只能映射到一个唯一的B集合。
	...
	解：就是括号问题。进栈是左括号，出栈是右括号，有多种进栈的方式就在任何时候，任何一个前缀上右括号数量不能比左括号要多，
	就是合法的进出栈方式，就是卡特兰数

		3.偶数个人排队, 假设个数是8, 那么一定有8个人排队, 排队的时候排两行, 要求任何一个后面的人都不比前面排队的人要小,
...
public class Code03_10Ways {
	...
	public static long ways2(int N) {
		...
		// c(2n,n) = 2n! / n!*n!，这里代码是考虑了n!可以约分。分母只计算n!,分子只计算2n*2n-1*...n+1
